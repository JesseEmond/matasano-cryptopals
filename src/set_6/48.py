...
r = rsa.Rsa(e=3, bits=768)


def pkcs_v1_oracle(ciphertext):
    plaintext = r.decrypt_bytes(ciphertext)
    try:
        pkcs1_v1_5.encrypt_unpad(plaintext, 768//8)
        return True
    except ValueError:
        return False


ciphertext = r.encrypt_bytes(pkcs1_v1_5.encrypt_pad(b"kick it, CC", 768//8))
e, n = r.public_key()
plaintext = rsa.attack_pkcs_v1_oracle(pkcs_v1_oracle, ciphertext, e, n,
                                      verbose=True)
plaintext = pkcs1_v1_5.encrypt_unpad(plaintext, 768//8)
print(plaintext)
